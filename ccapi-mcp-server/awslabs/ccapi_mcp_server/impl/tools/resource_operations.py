...
def _get_client_from_credentials_token(
    workflow_store: dict, credentials_token: str, service: str, region: str | None
):
    cred_data = validate_workflow_token(credentials_token, 'credentials', workflow_store)
    aws_session_data = cred_data['data']
    if not aws_session_data.get('credentials_valid'):
        raise ClientError('Invalid AWS credentials')

    region_str = ensure_region_string(region) or aws_session_data.get('region') or 'us-east-1'

    # Extract masked creds if present; this server design previously masked only for display
    # For explicit-credential flow, expect real creds to be present in session data under keys
    access_key = aws_session_data.get('aws_access_key_id') or environ.get('AWS_ACCESS_KEY_ID')
    secret_key = aws_session_data.get('aws_secret_access_key') or environ.get(
        'AWS_SECRET_ACCESS_KEY'
    )
    session_token = aws_session_data.get('aws_session_token') or environ.get('AWS_SESSION_TOKEN')

    if not access_key or not secret_key:
        raise ClientError('Credentials token missing explicit AWS credentials')

    return get_aws_client_with_credentials(
        service, region_str, access_key, secret_key, session_token
    )
...
async def create_resource_impl(request: CreateResourceRequest, workflow_store: dict) -> dict:
    ...
    # Validate credentials token and establish client using explicit credentials
    ...
    region_str = ensure_region_string(request.region) or aws_session_data.get('region') or 'us-east-1'
    cloudcontrol_client = _get_client_from_credentials_token(
        workflow_store, request.credentials_token, 'cloudcontrol', region_str
    )
    try:
...
async def get_resource_impl(
    ...
    # For get operations, require credentials token in request and use explicit client
    if workflow_store is None:
        raise ClientError('Internal error: workflow store is required')
    ...
    region_str = ensure_region_string(request.region) or aws_session_data.get('region') or 'us-east-1'
    cloudcontrol = _get_client_from_credentials_token(
        workflow_store, request.credentials_token, 'cloudcontrol', region_str
    )
    try:
        ...
        if request.analyze_security and workflow_store is not None:
            ...
            try:
                ...
                # Use existing credentials token rather than re-checking environment
                creds_token = request.credentials_token
...
async def get_resource_request_status_impl(request_token: str, region: str | None = None, *, workflow_store: dict | None = None, credentials_token: str | None = None) -> dict:
    ...
    if workflow_store is None or not credentials_token:
        raise ClientError('Credentials token required')
    ...
    region_str = ensure_region_string(region) or aws_session_data.get('region') or 'us-east-1'
    cloudcontrol_client = _get_client_from_credentials_token(
        workflow_store, credentials_token, 'cloudcontrol', region_str
    )
    try:
