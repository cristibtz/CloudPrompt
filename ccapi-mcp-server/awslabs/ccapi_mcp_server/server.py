...
• CREATE/UPDATE RULE: Never call generate_infrastructure_code() with empty properties. Provide minimally-required properties first. If missing info, ask one concise question or do a short web search, then proceed.
    - Examples of minimal properties:
        • AWS::EC2::Instance: ImageId, InstanceType (plus SubnetId/SecurityGroupIds if needed)
        • AWS::S3::Bucket: BucketName (optional if letting AWS generate), plus Tags
• LIST/GET/STATUS RULE: Always pass credentials_token to list_resources(), get_resource(), and get_resource_request_status().
...
async def list_resources(
    ...
    # Validate credentials token and create client with explicit credentials
    cred_data = validate_workflow_token(credentials_token, 'credentials', _workflow_store)
    ...
    cloudcontrol = get_aws_client_with_credentials(
        'cloudcontrol', region_str, aws_session_data.get('aws_access_key_id'), aws_session_data.get('aws_secret_access_key'), aws_session_data.get('aws_session_token')
    )
...
async def get_resource_request_status(
    ...
    return await get_resource_request_status_impl(
        request_token, region or 'us-east-1', workflow_store=_workflow_store, credentials_token=credentials_token
    )
...
async def check_environment_variables(
    ...
    region: str | None = Field(default=None, description='AWS region (optional; defaults to us-east-1)'),
) -> dict:
    """Validate explicit AWS credentials and create an environment token.

    This server requires explicit credentials; no environment/profile fallback is used.
    """
    return await check_environment_variables_impl(
        _workflow_store,
